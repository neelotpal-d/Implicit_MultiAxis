...
class Siren(nn.Module):
    def __init__(
        ...
        self.init_(weight, bias, c=c, w0=w0, scaled_init=scaled_init)
    ...
    def init_(self, weight, bias, c, w0, scaled_init=False):
        dim = self.dim_in
        if scaled_init:
            w_std = (1 / dim) if self.is_first else (math.sqrt(c / dim) / (1.5 * w0))  # changed here
        else:
            w_std = (1 / dim) if self.is_first else (math.sqrt(c / dim) / w0)  # changed here
...
class SirenNet(nn.Module):
    ...
    def forward(self, x, mods=None, compute_grads=True):
        ...
        if compute_grads:
            grads = torch.autograd.grad(out, inp, torch.ones_like(out), create_graph=True)[0]
            dx = grads[:, 0]
            dy = grads[:, 1]
            dz = grads[:, 2]
